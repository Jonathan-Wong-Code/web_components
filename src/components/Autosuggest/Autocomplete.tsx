...
const Li = styled.li<{ isHighlighted: boolean }>`
  cursor: pointer;
  background-color: ${({ isHighlighted }) => isHighlighted ? 'blue' : 'transparent'};
`
...
export const Autocomplete = ({ options = [], onChange }: IAutocomplete) => {
  ...
  const handleChange = (e: React.ChangeEvent) => {
    ...
    setShownOptions(() => options.filter(option => option.toLowerCase().includes(inputElement.value.toLowerCase())))
